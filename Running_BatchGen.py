...
class BatchRotateYCenterXZ(torch.nn.Module):
    ...
    def forward(self,global_positions,global_quats,ref_frame_id):
        a1 = global_positions[:, ref_frame_id:ref_frame_id+1, 5:6, :] - global_positions[:, ref_frame_id:ref_frame_id+1, 1:2, :]
        ref_vector = torch.cross(a1,
                                 torch.tensor([0, 1, 0], dtype=global_positions.dtype, device=global_positions.device).expand(a1[...,0:1,:].shape),dim=-1)
